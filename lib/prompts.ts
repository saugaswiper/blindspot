...
## 9. PRISMA-AI Transparency Checklist (NEW-10)

This review used AI-assisted tools at the following stages. Complete this section in your final manuscript to satisfy the forthcoming PRISMA-AI extension requirements (in development, PRISMA Group 2026).

### 9.1 AI Tools Used
| Stage | Tool | Version / Date | Purpose |
|---|---|---|---|
| Gap analysis | Blindspot (Google Gemini) | [version] | Identify evidence gaps and suggest review topics |
| Protocol drafting | Blindspot (Google Gemini) | [version] | Generate initial protocol skeleton |
| [Other stages] | [Tool name] | [version] | [Purpose] |

### 9.2 AI Validation Steps
- [ ] AI-generated gap identification verified against manual review of existing literature
- [ ] AI-suggested boolean search string tested in each database and adjusted by reviewers
- [ ] AI-generated protocol reviewed and adapted by at least one subject-matter expert
- [ ] All AI outputs documented with prompt text and response date for reproducibility
- [ ] Final protocol reviewed independently of AI outputs before PROSPERO registration

### 9.3 Human Oversight Declaration
*This protocol was AI-assisted but human-supervised. All AI outputs were critically evaluated and modified where necessary by the review team. The AI tools listed above did not make independent decisions about inclusion/exclusion criteria, evidence quality, or synthesis approach.*
...
Respond in this exact JSON format — no markdown, no code fences, just raw JSON:
{
  ...
  "embase_string": "A draft Embase Boolean search string using Embase-specific syntax. Use EMTREE descriptors (with / qualifier at end, e.g. 'depression/') combined with free-text (with :ti,ab/ suffix for title/abstract). Join with OR within concept blocks, AND between blocks. Example: ((depression/ OR 'major depressive disorder'/exp OR 'mood disorder':ti,ab/) AND ('cognitive behavior therapy'/exp OR 'CBT':ti,ab/) AND ('systematic review'/de OR 'meta-analysis'/de)). Include appropriate filters for publication type. Keep to 3-5 concept blocks. Output only the search string.",
  "central_string": "A draft Cochrane CENTRAL Boolean search string using CENTRAL/Cochrane Library syntax. Similar to PubMed with MeSH terms but formatted for the Cochrane interface. Use MeSH terms capitalized, combine with free-text synonyms, include review/protocol filters. Example: ((Depression/mesh OR Major Depressive Disorder OR mood disorder) AND (Cognitive Behavioral Therapy OR CBT) AND (Systematic Review OR Meta-Analysis)). This library uses slightly different syntax from PubMed but concepts are similar. Keep to 3-5 concept blocks. Output only the search string."
}`;
}
